...
export const fetchAllBookings = async () => {
  ...
  // Format data to match the expected structure from the MySQL server
  return data.map(booking => ({
    ...booking,
    slot_number: booking.parking_slots?.number
  }));
};
...
export const createTestBooking = async () => {
  ...
  if (slotsError || !slots || slots.length === 0) {
    ...
    throw slotsError || new Error('No available slots found');
  }
...
export const testDatabaseConnection = async () => {
  try {
    ...
    const { data, error } = await supabase.from('parking_slots.csv').select('count()', { count: 'exact' });
